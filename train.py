...
os.environ["CUDA_VISIBLE_DEVICES"]="5,6,8,9"
...
def train(generator, discriminator, init_step, loader, total_iter=600000):
    ...
    writer = tensorboardX.SummaryWriter("./%s/tensorboard"%(log_folder))
    ...
    for i in pbar:
        ...
        writer.add_scalar("Gen_loss_val", gen_loss_val,i)
        writer.add_scalar("disc_loss_val", disc_loss_val,i)
        writer.add_scalar("grad_loss_val", grad_loss_val,i)
...
if __name__ == '__main__':
    ...
    if args.resume_train:
        ...
        G_checkpoint = torch.load('/home/mjh/dgm/PGGAN-PyTorch/Progressive-GAN-pytorch/trial_experiment-2_2021-11-28_18_43/checkpoint/030000_g.model')
        D_checkpoint = torch.load('/home/mjh/dgm/PGGAN-PyTorch/Progressive-GAN-pytorch/trial_experiment-2_2021-11-28_18_43/checkpoint/030000_d.model')


        G = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm,
                      tanh=args.tanh).cuda()
        generator = nn.DataParallel(G).to(device)

        D = Discriminator(feat_dim=args.channel).cuda()

        discriminator = nn.DataParallel(D).to(device)

        g_running_cuda = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm,
                                   tanh=args.tanh).cuda()
        g_running = nn.DataParallel(g_running_cuda).to(device)
        ...
        # generator = torch.nn.DataParallel(Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh), device_ids=gpus).to(device)
        # discriminator = torch.nn.DataParallel(Discriminator(feat_dim=args.channel), device_ids=gpus).to(device)
        G = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm,tanh=args.tanh).cuda()
        ...
        g_running_cuda = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh).cuda()
        g_running = nn.DataParallel(g_running_cuda).to(device)
